...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds order id of smallest order' do
    ...
    # order_id = Order.minimum(:amount).limit(1).first.id
    # ------------------------------------------------------------
  ...
  it 'finds orders for a user' do
    ...
    # This way only looks through one table, since the user_id was given to us. Using the instance of the user
    # would have taken two tables
    # ------------------------------------------------------------
  ...
  it 'returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
     data = User
     .joins(:order_items)
     .select('name AS user_name, order_id, count(item_id) AS item_count')
